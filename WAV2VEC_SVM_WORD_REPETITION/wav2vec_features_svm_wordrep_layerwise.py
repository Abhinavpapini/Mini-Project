# ============================================================
# Wav2Vec2 Features + SVM (Clean vs Word Repetition) — SEP-28K
...
def main():
    print("\n" + "="*70)
    print("WAV2VEC2 FEATURES + SVM (WORD REPETITION CLASSIFICATION)")
    ...
    # Create dataset (Clean vs Word Repetition)
    print("[2] Creating Clean vs Word Repetition dataset...")
    ...
    print(f"Word Repetition samples: {len(wordrep_df)}")
    
    # Prepare file paths and labels
    file_paths = (
        [get_audio_path(r) for _, r in clean_df.iterrows()] +
        [get_audio_path(r) for _, r in wordrep_df.iterrows()]
    )
    
    labels = np.array([0] * len(clean_df) + [1] * len(wordrep_df))
    ...
    output_file = "results_wav2vec_svm_wordrep.txt"
    with open(output_file, 'w') as f:
        f.write("="*70 + "\n")
        f.write("WAV2VEC2 FEATURES + SVM (WORD REPETITION CLASSIFICATION)\n")
        f.write("CORRECTED METHODOLOGY: Wav2Vec2 as Feature Extractor\n")
        f.write("="*70 + "\n\n")
        
        f.write(f"Dataset:\n")
        f.write(f"  - Clean samples: {len(clean_df)}\n")
        f.write(f"  - Word Repetition samples: {len(wordrep_df)}\n")
