# ============================================================
# Wav2Vec2 + Classifier (Clean vs Prolongation) — SEP-28K
...
class Wav2Vec2Classifier(nn.Module):
    ...
    def forward(self, input_values):
        ...
        return self.classifier(torch.mean(outputs.last_hidden_state, dim=1))
...
def main():
    print("\n" + "="*60)
    print("WAV2VEC2 PROLONGATION CLASSIFICATION")
    print("="*60)
    ...
    prolongation_df = df[df["Prolongation"] > 0]
    
    clean_df = clean_df[clean_df.apply(lambda r: os.path.exists(get_audio_path(r)), axis=1)]
    prolongation_df = prolongation_df[prolongation_df.apply(lambda r: os.path.exists(get_audio_path(r)), axis=1)]
    
    print(f"Clean: {len(clean_df)} | Prolongation: {len(prolongation_df)}")
    
    file_paths = [get_audio_path(r) for _, r in clean_df.iterrows()] + [get_audio_path(r) for _, r in prolongation_df.iterrows()]
    labels = [0] * len(clean_df) + [1] * len(prolongation_df)
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if test_f1 > best_f1:
            best_f1 = test_f1
            torch.save(model.state_dict(), "best_wav2vec_prolongation_model.pt")
    
    model.load_state_dict(torch.load("best_wav2vec_prolongation_model.pt"))
    _, final_f1, final_acc, y_true, y_pred = evaluate(model, test_loader, criterion, DEVICE)
    print(f"\n🎯 Best F1: {best_f1:.4f}")
    print(classification_report(y_true, y_pred, target_names=['Clean', 'Prolongation']))
