# ============================================================
# Wav2Vec2 + Classifier (Clean vs Interjection) — SEP-28K
...
def main():
    print("\n" + "="*60)
    print("WAV2VEC2 INTERJECTION CLASSIFICATION")
    print("="*60)
    ...
    print("[2] Creating Clean vs Interjection dataset...")
    clean_df = df[
        (df["NoStutteredWords"] > 0) & (df["Block"] == 0) & (df["Prolongation"] == 0) &
        (df["SoundRep"] == 0) & (df["WordRep"] == 0) & (df["Interjection"] == 0)
    ]
    interjection_df = df[df["Interjection"] > 0]
    
    clean_df = clean_df[clean_df.apply(lambda r: os.path.exists(get_audio_path(r)), axis=1)]
    interjection_df = interjection_df[interjection_df.apply(lambda r: os.path.exists(get_audio_path(r)), axis=1)]
    
    print(f"Clean samples: {len(clean_df)}")
    print(f"Interjection samples: {len(interjection_df)}")
    
    file_paths = (
        [get_audio_path(r) for _, r in clean_df.iterrows()] +
        [get_audio_path(r) for _, r in interjection_df.iterrows()]
    )
    labels = [0] * len(clean_df) + [1] * len(interjection_df)
    ...
    print(f"\n[5] Initializing model on {DEVICE}...")
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if test_f1 > best_f1:
            best_f1 = test_f1
            torch.save(model.state_dict(), "best_wav2vec_interjection_model.pt")
            print(f"✅ Best model saved! (F1: {best_f1:.4f})")
    
    print(f"\n🎯 Best Test F1-Score: {best_f1:.4f}")
    
    model.load_state_dict(torch.load("best_wav2vec_interjection_model.pt"))
    _, final_f1, final_acc, y_true, y_pred = evaluate(model, test_loader, criterion, DEVICE)
    
    print("\n" + "="*60)
    print(f"Final Test F1: {final_f1:.4f} | Accuracy: {final_acc:.4f}")
    print("\n" + classification_report(y_true, y_pred, target_names=['Clean', 'Interjection']))
