# ============================================================
# Wav2Vec2 Features + SVM (Clean vs Prolongation) — SEP-28K
...
def main():
    print("\n" + "="*70)
    print("WAV2VEC2 FEATURES + SVM (PROLONGATION CLASSIFICATION)")
    ...
    # Create dataset (Clean vs Prolongation)
    print("[2] Creating Clean vs Prolongation dataset...")
    ...
    print(f"Prolongation samples: {len(prolongation_df)}")
    
    # Prepare file paths and labels
    file_paths = (
        [get_audio_path(r) for _, r in clean_df.iterrows()] +
        [get_audio_path(r) for _, r in prolongation_df.iterrows()]
    )
    
    labels = np.array([0] * len(clean_df) + [1] * len(prolongation_df))
    ...
    output_file = "results_wav2vec_svm_prolongation.txt"
    with open(output_file, 'w') as f:
        f.write("="*70 + "\n")
        f.write("WAV2VEC2 FEATURES + SVM (PROLONGATION CLASSIFICATION)\n")
        f.write("CORRECTED METHODOLOGY: Wav2Vec2 as Feature Extractor\n")
        f.write("="*70 + "\n\n")
        
        f.write(f"Dataset:\n")
        f.write(f"  - Clean samples: {len(clean_df)}\n")
        f.write(f"  - Prolongation samples: {len(prolongation_df)}\n")
