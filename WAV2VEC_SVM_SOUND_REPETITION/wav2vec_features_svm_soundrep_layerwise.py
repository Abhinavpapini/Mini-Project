# ============================================================
# Wav2Vec2 Features + SVM (Clean vs Sound Repetition) — SEP-28K
...
def main():
    print("\n" + "="*70)
    print("WAV2VEC2 FEATURES + SVM (SOUND REPETITION CLASSIFICATION)")
    ...
    # Create dataset (Clean vs Sound Repetition)
    print("[2] Creating Clean vs Sound Repetition dataset...")
    ...
    print(f"Sound Repetition samples: {len(soundrep_df)}")
    
    # Prepare file paths and labels
    file_paths = (
        [get_audio_path(r) for _, r in clean_df.iterrows()] +
        [get_audio_path(r) for _, r in soundrep_df.iterrows()]
    )
    
    labels = np.array([0] * len(clean_df) + [1] * len(soundrep_df))
    ...
    output_file = "results_wav2vec_svm_soundrep.txt"
    with open(output_file, 'w') as f:
        f.write("="*70 + "\n")
        f.write("WAV2VEC2 FEATURES + SVM (SOUND REPETITION CLASSIFICATION)\n")
        f.write("CORRECTED METHODOLOGY: Wav2Vec2 as Feature Extractor\n")
        f.write("="*70 + "\n\n")
        
        f.write(f"Dataset:\n")
        f.write(f"  - Clean samples: {len(clean_df)}\n")
        f.write(f"  - Sound Repetition samples: {len(soundrep_df)}\n")
