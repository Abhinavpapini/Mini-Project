# ============================================================
# Wav2Vec2 + Classifier (Clean vs Sound Repetition) — SEP-28K
...
def train_epoch(model, dataloader, criterion, optimizer, device):
    ...
    for batch in tqdm(dataloader, desc="Training"):
        ...
        optimizer.zero_grad()
        loss = criterion(model(input_values), labels)
        loss.backward()
        optimizer.step()
        total_loss += loss.item()
        all_preds.extend(torch.argmax(model(input_values), dim=1).cpu().numpy())
...
def main():
    print("\n" + "="*60)
    print("WAV2VEC2 SOUND REPETITION CLASSIFICATION")
    print("="*60)
    ...
    soundrep_df = soundrep_df[soundrep_df.apply(lambda r: os.path.exists(get_audio_path(r)), axis=1)]
    
    print(f"Clean: {len(clean_df)} | Sound Repetition: {len(soundrep_df)}")
    
    file_paths = [get_audio_path(r) for _, r in clean_df.iterrows()] + [get_audio_path(r) for _, r in soundrep_df.iterrows()]
    labels = [0] * len(clean_df) + [1] * len(soundrep_df)
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if test_f1 > best_f1:
            best_f1 = test_f1
            torch.save(model.state_dict(), "best_wav2vec_soundrep_model.pt")
    
    model.load_state_dict(torch.load("best_wav2vec_soundrep_model.pt"))
    _, final_f1, final_acc, y_true, y_pred = evaluate(model, test_loader, criterion, DEVICE)
    print(f"\n🎯 Best F1: {best_f1:.4f}")
    print(classification_report(y_true, y_pred, target_names=['Clean', 'SoundRep']))
