"""A4: Dimension-wise Gumbel hard layer selection with CNN classifier.
...
3) Learn per-dimension hard layer selection via straight-through Gumbel-Softmax
4) Classify selected feature vector with a 1D CNN head
5) Save metrics, per-dimension selection map, and training history
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="A4 dimension-wise gumbel layer selection")
...
class A4Model(nn.Module):
    def __init__(self, num_layers: int, feature_dim: int, dropout: float):
        super().__init__()
        # One distribution over layers per feature dimension: [D, L].
        self.dim_layer_logits = nn.Parameter(torch.zeros(feature_dim, num_layers))
    ...
    def forward(self, x: torch.Tensor, tau: float) -> Tuple[torch.Tensor, torch.Tensor]:
        # x: [B, L, D]
        # Hard-select layer per dimension via straight-through gumbel.
        sel = F.gumbel_softmax(self.dim_layer_logits, tau=tau, hard=True, dim=1)  # [D, L]
        x_perm = x.permute(0, 2, 1)  # [B, D, L]
        x_sel = torch.sum(x_perm * sel.unsqueeze(0), dim=2)  # [B, D]

        z = x_sel.unsqueeze(1)  # [B, 1, D]
...
def main() -> None:
    ...
    model = A4Model(num_layers=len(args.layers), feature_dim=d, dropout=args.dropout).to(device)
    ...
    with torch.no_grad():
        # Soft and hard per-dimension selections: [D, L].
        soft_sel = torch.softmax(model.dim_layer_logits, dim=1).cpu().numpy()
        hard_sel = F.gumbel_softmax(model.dim_layer_logits, tau=args.tau_end, hard=True, dim=1).cpu().numpy()

    # Per-layer aggregate share of selected dimensions.
    hard_counts = hard_sel.sum(axis=0)
    hard_share = hard_counts / hard_sel.shape[0]
    soft_mean = soft_sel.mean(axis=0)

    dim_csv = args.out_dir / "a4_dimension_selection_weights.csv"
    with dim_csv.open("w", newline="", encoding="utf-8") as f:
        fields = ["dim", *[f"soft_layer_{l}" for l in args.layers], "hard_selected_layer"]
        writer = csv.DictWriter(f, fieldnames=fields)
        writer.writeheader()
        hard_idx = np.argmax(hard_sel, axis=1)
        for dim_i in range(hard_sel.shape[0]):
            row = {"dim": int(dim_i)}
            for j, layer in enumerate(args.layers):
                row[f"soft_layer_{layer}"] = float(soft_sel[dim_i, j])
            row["hard_selected_layer"] = int(args.layers[int(hard_idx[dim_i])])
            writer.writerow(row)

    summary_csv = args.out_dir / "a4_layer_selection_summary.csv"
    with summary_csv.open("w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(
            f,
            fieldnames=["layer", "soft_mean_weight", "hard_selected_dims", "hard_selected_share"],
        )
        writer.writeheader()
        for j, layer in enumerate(args.layers):
            writer.writerow(
                {
                    "layer": int(layer),
                    "soft_mean_weight": float(soft_mean[j]),
                    "hard_selected_dims": int(hard_counts[j]),
                    "hard_selected_share": float(hard_share[j]),
                }
            )
    ...
    run_report = {
        ...
        "recall": float(rec),
        "soft_mean_layer_weights": {str(layer): float(w) for layer, w in zip(args.layers, soft_mean)},
        "hard_selected_share": {str(layer): float(s) for layer, s in zip(args.layers, hard_share)},
        "dominant_hard_layer": int(args.layers[int(np.argmax(hard_counts))]),
        "dimension_weights_csv": str(dim_csv),
        "summary_csv": str(summary_csv),
    }

    report_json = args.out_dir / "a4_run_report.json"
    report_json.write_text(json.dumps(run_report, indent=2), encoding="utf-8")

    history_csv = args.out_dir / "a4_train_history.csv"
    ...
    try:
        ...
        ax.bar([str(l) for l in args.layers], hard_share)
        ax.set_title(f"A4 Hard Dimension Selection Share ({args.target}, {args.model_alias})")
        ax.set_xlabel("Layer")
        ax.set_ylabel("Share of selected dimensions")
        ax.set_ylim(0.0, 1.0)
        fig.tight_layout()
        fig_path = args.fig_dir / "a4_layer_selection_share.png"
    ...
    print(f"Dimension weights: {dim_csv}")
