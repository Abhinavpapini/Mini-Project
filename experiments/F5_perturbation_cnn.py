"""F5: Acoustic Feature Perturbation Augmentation for robustness.

MOTIVATION: Raw audio augmentations (speed perturbation, pitch shift) cannot
be applied when working from cached HuBERT features. Instead, we simulate
their acoustic effects at the feature level:

  (1) Gaussian jitter — adds small N(0,σ) noise to PCA features
      Simulates: microphone/background noise, recording variation
      σ_jitter ~ U[0, 0.05] per sample (normalised feature scale)

  (2) Feature scaling — multiplies features by a scalar near 1.0
      Simulates: amplitude/volume variation (pitch-shift proxy)
      scale ~ U[0.85, 1.15] per sample

  (3) Gaussian dropout — randomly zeros individual features (p=0.1)
      Simulates: band-limited recordings, codec artefacts
      Distinct from DimMask (F2) — per-element, not block-based

All three are combined per training batch. Clean test features only.
Base: same PCA-32 → CNN-1D architecture as F1/F2.

Run command:
    python experiments/F5_perturbation_cnn.py \
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="F5: Acoustic perturbation + CNN-1D, multi-label")
    ...
    # Perturbation params
    p.add_argument("--jitter-sigma",   type=float,default=0.05,
                   help="Max std for Gaussian jitter (σ~U[0, jitter_sigma])")
    p.add_argument("--scale-range",    type=float,default=0.15,
                   help="Scale factor range: scale~U[1-range, 1+range]")
    p.add_argument("--dropout-p",      type=float,default=0.10,
                   help="Per-element feature dropout probability")
    # CNN params
    ...
    p.add_argument("--ckpt-dir", type=Path, default=Path("artifacts/checkpoints/F5"))
    return p.parse_args()
...
# ---------------------------------------------------------------------------
# Perturbation augmentation (training only)
# ---------------------------------------------------------------------------

def apply_perturbations(x: torch.Tensor, jitter_sigma: float,
                        scale_range: float, dropout_p: float) -> torch.Tensor:
    """
    Combine 3 acoustic-proxy perturbations per batch:
      1. Gaussian jitter (σ ~ U[0, jitter_sigma])
      2. Amplitude scaling (scale ~ U[1-range, 1+range])
      3. Per-element feature dropout (p=dropout_p)
    """
    # 1. Gaussian jitter
    if jitter_sigma > 0:
        sigma = float(np.random.uniform(0, jitter_sigma))
        x = x + sigma * torch.randn_like(x)

    # 2. Amplitude / volume scaling
    if scale_range > 0:
        scale = float(np.random.uniform(1.0 - scale_range, 1.0 + scale_range))
        x = x * scale

    # 3. Per-element feature dropout
    if dropout_p > 0:
...
# ---------------------------------------------------------------------------
# CNN-1D (same across all F-series)
# ---------------------------------------------------------------------------
...
def main() -> None:
    ...
    print(f"\n  Perturbations (train only):")
    print(f"    Gaussian jitter : σ ~ U[0, {args.jitter_sigma}]")
    print(f"    Amplitude scale : s ~ U[{1-args.scale_range:.2f}, {1+args.scale_range:.2f}]")
    print(f"    Feature dropout : p={args.dropout_p}")
    ...
    model    = CNN1DMultiLabel(in_dim=args.ssl_pca_dim, channels=channels,
                               num_classes=len(STUTTER_TYPES), dropout=args.dropout).to(device)
    n_params = sum(p.numel() for p in model.parameters())
    print(f"\nF5 model parameters: {n_params:,}")
    print(f"  CNN-1D: {args.ssl_pca_dim}→{channels}→5")
    ...
    # ------------------------------------------------------------------
    # 5. Training (with perturbations)
    # ------------------------------------------------------------------
    print(f"\nTraining for {args.epochs} epochs (perturbations: train only) ...")
    history: List[Dict] = []
    best_macro_f1 = -1.0
    best_ckpt = args.ckpt_dir / "f5_best.pt"
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for xb, yb in train_dl:
            ...
            xb = apply_perturbations(xb, args.jitter_sigma, args.scale_range, args.dropout_p)
            opt.zero_grad()
        ...
        # Evaluate on CLEAN test set (no perturbations)
        model.eval()
        vl, vlog, vlab = 0.0, [], []
        with torch.no_grad():
    ...
    # 7. Save
    # ------------------------------------------------------------------
    with (args.out_dir / "f5_perclass_results.csv").open("w", newline="", encoding="utf-8") as f:
    ...
    with (args.out_dir / "f5_train_history.csv").open("w", newline="", encoding="utf-8") as f:
    ...
    run_report = {
        "experiment": "F5",
        "title": "PCA-32 + Acoustic Perturbation + CNN-1D (HuBERT-large, feature robustness)",
        "device": str(device), "hubert_alias": args.hubert_alias,
        "hubert_layer": args.hubert_layer, "ssl_pca_dim": args.ssl_pca_dim,
        "jitter_sigma": args.jitter_sigma, "scale_range": args.scale_range,
    ...
    (args.out_dir / "f5_run_report.json").write_text(json.dumps(run_report, indent=2))
    ...
    try:
        ...
        axes[0].plot(ep_x, [r["train_loss"] for r in history], label="train (perturbed)")
        axes[0].plot(ep_x, [r["val_loss"]   for r in history], label="val (clean)")
        axes[0].set_title(f"F5 Loss (jitter={args.jitter_sigma}, scale±{args.scale_range})")
        axes[0].set_xlabel("Epoch"); axes[0].set_ylabel("Loss")
        axes[0].legend(); axes[0].grid(alpha=0.3)

        axes[1].plot(ep_x, [r["macro_f1"] for r in history], color="darkgreen", marker="o", ms=4)
        axes[1].axvline(x=history[best_ep_idx]["epoch"], ls="--", color="red", alpha=0.6,
                        label=f"best={best_macro_f1:.4f}")
        axes[1].set_title("F5 Validation Macro-F1 (Perturbation Robustness)")
        axes[1].set_xlabel("Epoch"); axes[1].set_ylabel("Macro-F1")
        axes[1].legend(); axes[1].grid(alpha=0.3)
        fig.tight_layout()
        fig.savefig(args.fig_dir / "f5_train_curves.png", dpi=160); plt.close(fig)
        ...
        ax2.set_ylim(0, 1)
        ax2.set_title(f"F5 Per-class F1 (PCA-{args.ssl_pca_dim} + Perturbation)")
        ax2.set_xlabel("Stutter Type"); ax2.set_ylabel("F1"); ax2.legend()
        fig2.tight_layout()
        fig2.savefig(args.fig_dir / "f5_perclass_f1.png", dpi=160); plt.close(fig2)
        print(f"\n  Saved: {args.fig_dir / 'f5_train_curves.png'}")
        print(f"  Saved: {args.fig_dir / 'f5_perclass_f1.png'}")
    except Exception as exc:
        print(f"  [WARN] Figure generation failed: {exc}")

    print("\n✅  F5 complete.")
    print(f"   This is the FINAL experiment (35/35). All core experiments done!")
    print(f"   Report : {args.out_dir / 'f5_run_report.json'}")
