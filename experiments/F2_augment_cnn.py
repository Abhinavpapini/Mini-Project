"""F2: Feature-space data augmentation benchmark.

Applies two complementary augmentation strategies IN EMBEDDING SPACE
to improve generalisation without requiring raw audio:

  (1) Mixup — convex interpolation of feature vectors + labels:
        x̃ = λ·xᵢ + (1-λ)·xⱼ,  ỹ = λ·yᵢ + (1-λ)·yⱼ
        λ ~ Beta(α, α), α=0.3  (small α → mixtures stay close to originals)

  (2) Feature Masking (SpecAugment-style on 1D features):
        Randomly zeros contiguous blocks of PCA dimensions
        (analogous to frequency masking in SpecAugment)
        mask_ratio ~ U[0, 0.2] → up to 20% of 32 dims masked per sample

Both augmentations are applied ONLY during training.
Architecture is otherwise identical to F1 (PCA-32 + CNN-1D).

Run command:
    python experiments/F2_augment_cnn.py \
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="F2: Augmented embedding-space CNN-1D")
    ...
    p.add_argument("--mixup-alpha",    type=float,default=0.3,
                   help="Beta distribution α for mixup. 0 = disabled.")
    p.add_argument("--mask-ratio",     type=float,default=0.2,
                   help="Max fraction of feature dims to zero-mask per sample.")
    p.add_argument("--mask-block-size",type=int,  default=4,
                   help="Contiguous block size for dimension masking (SpecAugment-style).")
    # CNN params
    ...
    p.add_argument("--ckpt-dir", type=Path, default=Path("artifacts/checkpoints/F2"))
    return p.parse_args()
...
# ---------------------------------------------------------------------------
# Augmentation functions (applied in training loop, GPU-side)
# ---------------------------------------------------------------------------

def apply_mixup(x: torch.Tensor, y: torch.Tensor,
                alpha: float) -> Tuple[torch.Tensor, torch.Tensor]:
    """Convex interpolation of feature/label pairs."""
    if alpha <= 0:
        return x, y
    lam = float(np.random.beta(alpha, alpha))
    idx = torch.randperm(x.size(0), device=x.device)
    x_mix = lam * x + (1 - lam) * x[idx]
    y_mix = lam * y + (1 - lam) * y[idx]   # soft labels for MLSM
    return x_mix, y_mix


def apply_dim_mask(x: torch.Tensor, mask_ratio: float,
                   block_size: int) -> torch.Tensor:
    """Zero-out contiguous blocks of feature dimensions (SpecAugment-style)."""
    if mask_ratio <= 0:
        return x
    n_dims = x.size(1)
    n_mask = max(1, int(n_dims * mask_ratio))
    # Random block start (ensure block fits within dims)
    max_start = max(0, n_dims - block_size)
    start = np.random.randint(0, max_start + 1)
    end   = min(start + n_mask, n_dims)
    x_masked = x.clone()
...
# ---------------------------------------------------------------------------
# CNN-1D (same architecture as F1/B1)
# ---------------------------------------------------------------------------
...
def main() -> None:
    ...
    print(f"\nF2 model parameters: {n_params:,}")
    print(f"  CNN-1D: {args.ssl_pca_dim}→{channels}→5")
    print(f"  Augmentation: mixup(α={args.mixup_alpha}) + dim-mask(ratio={args.mask_ratio}, "
          f"block={args.mask_block_size})")
    ...
    # ------------------------------------------------------------------
    print(f"\nTraining for {args.epochs} epochs (augmentation: train only) ...")
    history: List[Dict] = []
    best_macro_f1 = -1.0
    best_ckpt = args.ckpt_dir / "f2_best.pt"
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for xb, yb in train_dl:
            ...
            # Apply feature masking (per-batch, stochastic)
            xb = apply_dim_mask(xb, args.mask_ratio, args.mask_block_size)
            # Apply mixup (in-place over the masked batch)
            xb, yb = apply_mixup(xb, yb, args.mixup_alpha)
            opt.zero_grad()
        ...
        with torch.no_grad():
            for xb, yb in test_dl:
                xb, yb = xb.to(device), yb.to(device)
                lg = model(xb)      # no augmentation at eval
    ...
    # 7. Save
    # ------------------------------------------------------------------
    with (args.out_dir / "f2_perclass_results.csv").open("w", newline="", encoding="utf-8") as f:
    ...
    with (args.out_dir / "f2_train_history.csv").open("w", newline="", encoding="utf-8") as f:
    ...
    (args.out_dir / "f2_run_report.json").write_text(json.dumps(run_report, indent=2))
    ...
    try:
        ...
        axes[0].plot(ep_x, [r["train_loss"] for r in history], label="train (augmented)")
        axes[0].plot(ep_x, [r["val_loss"]   for r in history], label="val (clean)")
        axes[0].set_title(f"F2 Loss (mixup α={args.mixup_alpha}, mask={args.mask_ratio})")
        axes[0].set_xlabel("Epoch"); axes[0].set_ylabel("Loss")
        axes[0].legend(); axes[0].grid(alpha=0.3)

        axes[1].plot(ep_x, [r["macro_f1"] for r in history], color="indigo", marker="o", ms=4)
        axes[1].axvline(x=history[best_ep_idx]["epoch"], ls="--", color="red", alpha=0.6,
                        label=f"best={best_macro_f1:.4f}")
        axes[1].set_title("F2 Validation Macro-F1")
        axes[1].set_xlabel("Epoch"); axes[1].set_ylabel("Macro-F1")
        axes[1].legend(); axes[1].grid(alpha=0.3)
        fig.tight_layout()
        fig.savefig(args.fig_dir / "f2_train_curves.png", dpi=160); plt.close(fig)
        ...
        ax2.set_ylim(0, 1)
        ax2.set_title(f"F2 Per-class F1 (PCA-{args.ssl_pca_dim} + Mixup + DimMask)")
        ax2.set_xlabel("Stutter Type"); ax2.set_ylabel("F1"); ax2.legend()
        fig2.tight_layout()
        fig2.savefig(args.fig_dir / "f2_perclass_f1.png", dpi=160); plt.close(fig2)
        print(f"\n  Saved: {args.fig_dir / 'f2_train_curves.png'}")
        print(f"  Saved: {args.fig_dir / 'f2_perclass_f1.png'}")
    except Exception as exc:
        print(f"  [WARN] Figure generation failed: {exc}")

    print("\n✅  F2 complete.")
    print(f"   Report : {args.out_dir / 'f2_run_report.json'}")
